...
before(() => {
  findOrCreateFixtureDepartment().then(({ id, hasDirector }) => {
    ...
    createDirectorAccount('historial').then((account) => {
      ...
      // Un solo login real por UI para todo el spec, no uno por test:
      // Firebase limita cuántos `signInWithPassword` acepta por proyecto en
      // una ventana de tiempo (ver E2E.md "Problemas conocidos"). La sesión
      // que deja este login vive en la IndexedDB de Firebase, que Cypress no
      // limpia entre pruebas (solo cookies/localStorage), así que sigue ahí
      // para el resto del spec — ver el `beforeEach` de más abajo.
