...
import useGetTeacherHistoryStats from "../../hooks/useGetTeacherHistoryStats";
...
function TeacherChartsSection() {
  const {
    data: statsResponse,
    isLoading,
    isFetched,
  } = useGetTeacherHistoryStats();

  const stats = statsResponse?.data ?? [];

  const historicalData: ChartDataPoint[] = stats.map((s) => ({
    label: s.period_code,
    value: s.overall_average ?? 0,
  }));
...
export default TeacherChartsSection;
