...
import { useAssignDean } from '../api'
import type { Faculty } from '../types'

interface AssignDeanDrawerProps {
  /** Faculty to assign a dean to. Pass null to close the drawer. */
...
/**
 * Drawer that lists active users — searchable and paginated, with no role
 * restriction, since a dean is not necessarily a teacher or a director
 * beforehand — so the user can pick one and assign them as dean of the
 * selected faculty. The backend grants the `DECANO` role automatically.
 *
 * @example
 * <AssignDeanDrawer faculty={faculty} open onOpenChange={setOpen} />
 */
export function AssignDeanDrawer({ faculty, open, onOpenChange }: AssignDeanDrawerProps) {
  ...
  const { mutate: assignDean, isPending: isAssigning } = useAssignDean()
  ...
  const handleAssign = () => {
    if (!faculty || selectedUserId == null) return

    assignDean(
      { facultyId: faculty.id, userId: selectedUserId },
      {
        onSuccess: () => {
          toast.success('Decano asignado exitosamente')
          onOpenChange(false)
        },
      },
    )
  ...
  return (
    <Drawer open={open} onOpenChange={onOpenChange} swipeDirection="right">
      <DrawerContent className="w-full sm:max-w-xl">
        <DrawerHeader>
          <DrawerTitle>Asignar decano</DrawerTitle>

          <DrawerDescription>
            {faculty
              ? `Selecciona un decano para la facultad ${faculty.name}.`
              : 'Selecciona un decano para la facultad.'}
          </DrawerDescription>
        </DrawerHeader>
        ...
        <DrawerFooter className="border-t">
          ...
          >
            ...
            {isAssigning ? 'Asignando...' : 'Asignar decano'}
          </Button>
        </DrawerFooter>
      </DrawerContent>
    </Drawer>
