  ...
  if (!plan) {
    return (
      <Card className="p-5 sm:p-6">
        <h2 className="text-ink-900 text-[20px] font-semibold">Plan de Mejoramiento Docente</h2>

        <p className="text-ink-500 mt-2 text-[13px]">
          No tienes planes de mejoramiento activos. Si tu director registra un plan de seguimiento
      ...
      <div className="mb-5 flex flex-col justify-between gap-3 sm:flex-row sm:items-end">
        <div>
          ...
          <p className="text-ink-500 mt-1 text-[13px]">
            {plan.title} · Origen {plan.origin_period_code ?? '—'} → Verificación{' '}
        ...
        <div className="flex items-center gap-3">
          ...
          <Link
            href="/my-plans"
            className="text-foreground hover:bg-muted inline-flex h-9 items-center gap-1.5 rounded-md border px-3 text-[13px] font-semibold"
          >
            Ver mi plan
            <ArrowRight size={14} />
          </Link>
        ...
        {plan.checkpoints.map((cp) => {
          ...
          return (
            <div key={cp.id} className="rounded-lg border p-3 text-center">
              ...
              <div className="text-ink-900 mt-2 text-[13px] font-semibold">
                {STAGE_LABEL[cp.stage] ?? cp.stage}
              </div>

              <div className="text-ink-500 mt-0.5 text-[11.5px] font-medium tracking-[0.04em] uppercase">
