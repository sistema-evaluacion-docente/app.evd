...
const PERIODS: PeriodRecord[] = [
  {
    ...
    score: 4.8,
    comments: 38,
    dims: { desempeno: 92, desarrollo: 88, procesos: 95, integracion: 91 },
  },
  {
    id: '2023-2',
    period: '2023-2',
    score: 4.7,
    comments: 42,
    dims: { desempeno: 90, desarrollo: 86, procesos: 92, integracion: 90 },
  },
  {
    id: '2023-1',
    period: '2023-1',
    score: 4.5,
    comments: 36,
    dims: { desempeno: 85, desarrollo: 82, procesos: 88, integracion: 86 },
  },
  {
    id: '2022-2',
    period: '2022-2',
    score: 4.6,
    comments: 33,
    dims: { desempeno: 88, desarrollo: 84, procesos: 90, integracion: 87 },
  },
  {
    id: '2022-1',
    period: '2022-1',
    score: 4.4,
    comments: 29,
    dims: { desempeno: 82, desarrollo: 80, procesos: 86, integracion: 83 },
  },
  {
    id: '2021-2',
    period: '2021-2',
    score: 4.2,
    comments: 25,
    dims: { desempeno: 80, desarrollo: 78, procesos: 84, integracion: 81 },
  },
]
...
export function MyHistoryPage() {
  ...
  const dimSeries = [
    {
      ...
      color: '#0F172A',
      values: CHRONO.map((p) => p.dims.desempeno),
    },
    {
      key: 'desarrollo',
      label: 'Desarrollo del Conocimiento',
      color: '#2A6FDB',
      values: CHRONO.map((p) => p.dims.desarrollo),
    },
    {
      key: 'procesos',
      label: 'Procesos de Evaluación',
      color: '#1F8A5B',
      values: CHRONO.map((p) => p.dims.procesos),
    },
    {
      key: 'integracion',
      label: 'Integración Interpersonal',
      color: '#D97757',
      values: CHRONO.map((p) => p.dims.integracion),
    },
  ]
  ...
  const sortHeader = (label: string, key: SortKey) => (
    <button
      ...
      className="inline-flex items-center gap-1.5 tracking-[0.08em] uppercase"
    >
      {label}
      <ArrowUpDown size={11} className={cn('opacity-60', sortKey === key && 'opacity-100')} />
    </button>
  )
  ...
  const columns: DataTableColumn<PeriodRecord>[] = [
    {
      ...
      cell: (period) => (
        <input
          ...
          className="border-ink-300 accent-brand-600 h-4 w-4 rounded"
    ...
    },
    {
      ...
      cell: (period) => (
        ...
        >
          ...
          />
          <span className="text-ink-900 text-[15px] font-semibold">{period.period}</span>
        </button>
      ),
    },
    {
      ...
      cell: (period) => {
        ...
        return (
          <div className="flex items-center gap-3">
            <span className="num text-ink-900 w-10 text-[15px] font-semibold tabular-nums">
              {period.score.toFixed(1)}
            </span>
            <div className="bg-ink-100 h-1.5 overflow-hidden rounded-full" style={{ width: 80 }}>
              <div
    ...
    },
    {
      ...
      cell: (period) => {
        ...
        return (
          <span
            className={cn(
              'inline-flex h-7 items-center rounded-full border px-3 text-[11px] font-semibold whitespace-nowrap',
              level.bg,
              level.text,
              level.border,
            )}
    ...
    },
    {
      ...
      cell: (period) => {
        ...
        if (delta === null) {
          return <span className="text-ink-400 text-[12.5px]">—</span>
        }
        return delta >= 0 ? (
          ...
          <span className="text-brand-700 inline-flex items-center gap-1 text-[12.5px] font-semibold">
    ...
    },
    {
      header: 'Comentarios',
      cell: (period) => (
        <span className="num text-ink-700 text-[13.5px] tabular-nums">{period.comments}</span>
      ),
    },
    {
      header: 'Acción',
      ...
      cell: () => (
        <Link
          href="/me/summary"
          className="bg-primary text-primary-foreground hover:bg-primary-hover inline-flex h-8 items-center justify-center rounded-md px-3 text-[12.5px] font-semibold"
        >
          Ver Detalle
        </Link>
      ),
  ...
  return (
    <AppLayout header={{ userName: 'Dr. Roberto Jiménez', userRole: 'Facultad de Ingeniería' }}>
      <PageHeader
        underline
      ...
      <Card className="p-5 sm:p-6">
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
          ...
          <div className="flex items-center gap-3">
            <div className="bg-ink-100 flex items-center rounded-md p-0.5">
        ...
        <Card className="border-brand-200 bg-brand-50/50 p-4 sm:p-5">
          <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
            <div className="flex flex-wrap items-center gap-2">
              <span className="text-brand-700 text-[11px] font-semibold tracking-[0.06em] uppercase">
                Comparando
              </span>
              {compareIds.map((id) => {
                ...
                return (
                  <span
                    key={id}
                    className="border-brand-200/70 bg-card inline-flex h-7 items-center gap-1.5 rounded-full border pr-1.5 pl-2.5 text-[12.5px]"
                  >
                    <span className="text-ink-900 font-semibold">{period.period}</span>
                    <span className="num text-ink-500">· {period.score.toFixed(1)}</span>
                    <button
                      type="button"
                      onClick={() => toggleCompare(id)}
                      className="text-ink-500 hover:bg-ink-100 inline-flex h-5 w-5 items-center justify-center rounded-full"
                      aria-label="Quitar"
                    >
                      <X size={11} />
                    </button>
      ...
      {/* Level filter */}
      <div className="flex flex-wrap items-center gap-2">
        <span className="text-ink-500 mr-1 text-[11px] font-semibold tracking-[0.06em] uppercase">
          Filtrar por nivel
        </span>
        <FilterPills
          value={levelFilter}
      ...
      <Card className="overflow-hidden">
        <DataTable
          ...
          headerVariant="dark"
          minWidth={820}
          rowClassName={(period) => cn('h-[64px]', expandedId === period.id && 'bg-ink-50/40')}
          emptyMessage="Sin resultados para este filtro."
          expandedKey={expandedId}
          renderExpanded={(period) => <PeriodDetail period={period} />}
        />
        <div className="border-ink-100 text-ink-500 flex items-center justify-between border-t px-5 py-3.5 text-[12px]">
          <span>
            {filtered.length} {filtered.length === 1 ? 'periodo' : 'periodos'} mostrados
          </span>
      ...
      <AppFooter>Dr. Roberto Jiménez · Sistema de Evaluación Docente · v2.1</AppFooter>
    </AppLayout>
  )
}
...
function PeriodDetail({ period }: { period: PeriodRecord }) {
  ...
  return (
    <div className="bg-ink-50/40 grid grid-cols-1 gap-x-8 gap-y-4 px-6 pt-4 pb-5 md:grid-cols-2">
        ...
        {dims.map((dimension) => (
          <div key={dimension.label}>
            <div className="flex items-center justify-between text-[12px] font-medium">
              ...
              <span className="num text-ink-900 font-semibold tabular-nums">
                {dimension.value}%
              </span>
            </div>
            <div className="bg-ink-100 mt-1 h-1.5 overflow-hidden rounded-full">
              <div className="bg-ink-800 h-full" style={{ width: `${dimension.value}%` }} />
            </div>
          </div>
        ))}
      </div>
      <div className="space-y-3">
        ...
        <ul className="text-ink-700 space-y-1.5 text-[13px]">
          <li className="flex justify-between">
            <span>Comentarios recibidos</span>
            <span className="num text-ink-900 font-semibold tabular-nums">{period.comments}</span>
          </li>
          <li className="flex justify-between">
          ...
          <li className="flex justify-between">
            ...
            <span className="text-ink-900 font-medium">Procesos de Evaluación</span>
          </li>
          <li className="flex justify-between">
        ...
        <Link
          href="/me/summary"
          className="text-brand-700 hover:text-brand-800 inline-flex items-center gap-1.5 text-[12.5px] font-semibold"
        >
